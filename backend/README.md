...
Server defaults to `http://localhost:3000`. Postgres is published on port `5434` to avoid clashing with other local databases.
...
The Next.js client lives in [`../frontend`](../frontend) with its own `package.json`, env files, lint config, and setup docs.
